...
function v2(f){
    ...
    var reg = (fo.ext == '.css') ? reg2 : reg3

    var body = fn.getBody(f).replace(reg, (a, b)=>{
        if (b){
            return vf(f, a, b)
        }
        else {
...
function v3(f){
    ...
    var v = ''
    var isStatic = rf.split('/')[0] == 'static'
    if (confr.htmlVersion && isStatic){
        v = confr.env.htmlVersion
    }
...
function version(){
    ...
    fn.walk(fromDir, f=>{
        if (isCopy(f)){
            ps.push(fn.copy(f, path.join(toDir, path.relative(fromDir, f))))
        }
        else {
            iftable(f, ftable)
        }
...
function watch(){
    ...
    fs.watch(fromDir, {recursive:true}, (event, f)=>{
        ...
        if (isCopy(f)){
            fn.copy(f, path.join(toDir, path.relative(fromDir, f)))
            return
        }
