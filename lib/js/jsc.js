...
function compileJs(f){
    ...
    var bf = path.basename(f)

    //忽略.min.js
    if (bf.slice(-7) == '.min.js'){
        fn.createF(path.join(toDir, f.replace(fromDir, '')), fn.getBody(f))
        return
    }
...
function watch(){
    fs.watch(fromDir, {recursive:true}, (event, f) => {
        if (f && (f[0]!='.') && (path.extname(f) == '.js') ){

            f = path.join(fromDir, f)
            console.log(`watch change: ${f}`)
